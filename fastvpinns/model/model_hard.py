...
class DenseModel(tf.keras.Model):
    ...
    def call(self, inputs):
        ...
        k = 10.0 / 1e-8
        x = x * (
            (1 - tf.exp(-k * inputs[:, 0:1]))
            * (1 - tf.exp(-k * inputs[:, 1:2]))
            * (1 - tf.exp(-k * (1 - inputs[:, 0:1])))
            * (1 - tf.exp(-k * (1 - inputs[:, 1:2])))
        )
        return x
    ...
    @tf.function
    def train_step(self, beta=10, bilinear_params_dict=None, l2_lambda=0.0001):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            l2_regularisation = l2_lambda * tf.reduce_sum(
                [
                    tf.nn.l2_loss(var)
