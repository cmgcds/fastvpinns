    ...
    def __init__(
        ...
        for k, v in self.orig_factor_matrices.items():
            for kk, vv in v.items():
                print(
                    f"| {str(kk) + '['+str(k)+']':<25} | {str(vv.shape):<25} | {vv.dtype}"
                )

        # loop over the boundary conditions to obtain the shape of the boundary data
        for i in range(len(self.dirichlet_input_list)):
            print(
                f"| {'dirichlet_input['+str(i)+']':<25} | {str(self.dirichlet_input_list[i].shape):<25} | {self.dirichlet_input_list[i].dtype}"
            )
            print(
                f"| {'dirichlet_actual['+str(i)+']':<25} | {str(self.dirichlet_actual_list[i].shape):<25} | {self.dirichlet_actual_list[i].dtype}"
            )
    ...
    def train_step(self, beta=10, bilinear_params_dict=None):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            boundary_loss += tf.reduce_mean(
                tf.square(u_predicted_boundary - self.dirichlet_actual_list[0]), axis=0
            )
            boundary_loss += 10.0 * tf.reduce_mean(
                tf.square(v_predicted_boundary - self.dirichlet_actual_list[1]), axis=0
            )
