...
class DataHandler2D_Vector:
    ...
    def __init__(self, fespace_list, fespace_name_list, domain, dtype):
        ...
        if len(self.fespace_list) != len(self.fespace_name_list):
            print(
                "[ERROR] The length of the fespace_list and fespace_dict are not equal, File :",
                __file__,
            )
            raise ValueError(
                "The length of the fespace_list and fespace_dict are not equal"
            )
        ...
        for fespace_name, fespace in self.fespace_dict.items():
            ...
            for cell_index in range(fespace.n_cells):
                ...
                x_pde = tf.constant(
                    fespace.get_quadrature_actual_coordinates(cell_index),
                    dtype=self.dtype,
                )
                quad_weight = tf.constant(
            ...
            self.quad_weight_list = tf.transpose(
                tf.stack(self.quad_weight_list, axis=1)
            )
    ...
    def get_dirichlet_input(self, component_list, fespaces_list):
        ...
        if len(component_list) != len(fespaces_list):
            ...
            raise ValueError(
                "The length of the component_list and fespaces_list are not equal"
            )

        for fespace, component in zip(fespaces_list, component_list):
            input_dirichlet, actual_dirichlet = (
                fespace.generate_dirichlet_boundary_data_vector(component)
            )
