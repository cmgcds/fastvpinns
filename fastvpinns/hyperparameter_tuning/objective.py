...
def objective(trial):
    ...
    config = {
        ...
        'model': {
            'model_architecture': [2] + [trial.suggest_int(f'layer_{i}', 10, 100) for i in range(trial.suggest_int('n_layers', 1, 5))] + [1],
            'activation': 'tanh',
            'use_attention': False,
            'learning_rate': {
    ...
    domain = Geometry_2D(
        "quadrilateral", "internal", 100, 100, output_temp_dir
    )
