...
def test_quadrature_uniform(coord, quad_type, transformation):
  ...
  cells, boundary_points = domain.generate_quad_mesh_internal(
      x_limits=[coord[0],coord[1]], y_limits=[coord[2], coord[3]], n_cells_x=4, n_cells_y=4, num_boundary_points=100
  )
  ...
  assert np.isclose(int_sum, actual_area, rtol=1e-4), f"Failed for quad_type: {quad_type}, transformation: {transformation}"
...
def test_quadrature_complex( quad_type, transformation):
  ...
  assert np.isclose(int_sum, actual_area, rtol=1e-2), f"Failed for quad_type: {quad_type}, transformation: {transformation}"
  print(f"Test passed for quad_type: {quad_type}, transformation: {transformation}")
