...
if __name__ == "__main__":
    ...
    model = DenseModel_NSE2D_Scaling(
        ...
        # loss_function=pde_loss_nse2d_fs,
    ...
    np.savetxt(str(Path(i_output_path) / ".txt"), y_pred_p)
    ...
    np.savetxt(str(Path(i_output_path) / "pde_loss.txt"), np.array(pde_loss_array))  # pde loss
    ...
    os.system(f"cp fs.py {i_output_path}")
    ...
    os.system(f"cp ../../../../../fastvpinns/physics/nse2d_fs.py {i_output_path}")
    ...
    if i_use_wandb:
        ...
        # save the sub files
        wandb.save(str(Path(i_output_path) / "main_ns2d_fs.py"))
        wandb.save(str(Path(i_output_path) / "fs.py"))
        wandb.save(str(Path(i_output_path) / "model_nse2d.py"))
        wandb.save(str(Path(i_output_path) / "nse2d_fs.py"))

    # Plot the Final plot, similar to the one mentioned in Paper.
    names = ["U", "V", "P"]
    pred = [
        y_pred_u.reshape(original_shape),
        y_pred_v.reshape(original_shape),
        y_pred_p.reshape(original_shape),
    ]
    ref = [Y_Exact_Matrix_u, Y_Exact_Matrix_v, Y_Exact_Matrix_p]
    error = [error_u, error_v, error_p]
    for i, name in enumerate(names):
        fig, axs = plt.subplots(3, figsize=(9, 10))
        axs[0].contourf(X, Y, pred[i])
        axs[1].contourf(X, Y, ref[i])
        clb = axs[2].contourf(X, Y, error[i])
        cbar = plt.colorbar(clb, orientation="horizontal", pad=0.3)
        axs[0].set_title(f"{names[i]} Prediction", fontdict={"size": 18})
        axs[1].set_title("Reference", fontdict={"size": 18})
        axs[2].set_title("Error", fontdict={"size": 18})
        cbar.ax.locator_params(nbins=5)
        cbar.ax.tick_params(labelsize=12)
        plt.tight_layout()
        plt.savefig(f"{i_output_path}/final_plot_{name}.png")
