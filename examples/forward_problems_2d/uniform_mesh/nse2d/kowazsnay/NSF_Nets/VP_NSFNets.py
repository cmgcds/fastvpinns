...
# set tensorflow v1 for compatibility
...
class VPNSFnet:
    ...
    def __init__(self, xb, yb, ub, vb, x, y, layers):
        ...
        self.u_boundary_pred, self.v_boundary_pred, self.p_boundary_pred = self.net_NS(
            self.x_boundary_tf, self.y_boundary_tf
        )
        ...
        self.loss = (
            alpha * tf.reduce_mean(tf.square(self.u_boundary_tf - self.u_boundary_pred))
    ...
    def xavier_init(self, size):
        ...
        return tf.Variable(
            tf.random.truncated_normal([in_dim, out_dim], stddev=xavier_stddev), dtype=tf.float32
        )
    ...
    def Adam_train(self, nIter=5000, learning_rate=1e-3):
        ...
        print("Adam training time: ", main_end_time - main_start_time, "s")
...
if __name__ == "__main__":
    ...
    model = VPNSFnet(xb_train, yb_train, ub_train, vb_train, x_train, y_train, layers)

    # train the adam model for 1000
    # import time

    time_val = model.Adam_train(1000, 1e-3)

    from datetime import datetime

    now = datetime.now()
    dateprefix = now.strftime("%d_%b_%Y_%H_%M")
    run_name = "NSF_Net_Timing_7_100" + "_" + dateprefix
    wandb.init(project="ICCFD_Kovasznay", entity="starslab-iisc", name=run_name, config={})
    wandb.log({"Adam Training Time (Total)": time_val})
    wandb.log({"Adam Training Time (Per Iteration)": (time_val) / 1000})
