...
if __name__ == "__main__":
    ...
    if i_exact_solution_generation == "external":
        y_exact = np.loadtxt(i_exact_solution_file_name)
        y_exact_u = y_exact[:, 2]
        y_exact_v = y_exact[:, 3]
        y_exact_p = y_exact[:, 4]
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % i_update_console_output == 0 or epoch == num_epochs - 1:
            ...
            # compute the mean diffence in pressure b.w exact and predicted
            diff = np.mean(y_exact_p) - np.mean(y_pred_p)
            y_pred_p = y_pred_p + diff
    ...
    os.system(f"cp kovasznay.py {i_output_path}")
    ...
    if i_use_wandb:
        ...
        wandb.log(
            {
                "Time per Epoch(s) - Median": np.median(time_array),
                "Time per Epoch(s) IQR-25%": np.percentile(time_array, 25),
                "Time per Epoch(s) IQR-75%": np.percentile(time_array, 75),
                "Mean (s)": np.mean(time_array),
                "Epochs per second": int(i_epochs / np.sum(time_array)),
                "Total Train Time": np.sum(time_array),
            }
        )

        # wandb log penalty coefficients
        wandb.log(penalty_coefficients_dict)

        # wandb log bilinear parameters
        wandb.log(bilinear_params_dict)
        ...
        wandb.save(str(Path(i_output_path) / "kovasznay.py"))
