...
from fastvpinns.physics.nse2d_inverse import pde_loss_nse2d_inverse
...
from inverse_backward_step import *


# Modified to get the sensor data points also
def get_dirichlet_and_test_data_external(filename, dtype, num_sensor_points):
    """
    ...
    # lets pick the sensor values
    collocation_points = np.vstack((x.flatten(), y.flatten())).T
    solution_at_collocation_points = np.vstack((u.flatten(), v.flatten(), p.flatten())).T

    # pick points which are less than x = 9 and greater than x = 0
    # and in the range of y = 0.45 and y = -0.45
    ind = (
        (collocation_points[:, 0] < 18)
        & (collocation_points[:, 0] > 0)
        & (collocation_points[:, 1] < 0.48)
        & (collocation_points[:, 1] > -0.48)
    )
    collocation_points = collocation_points[ind]
    solution_at_collocation_points = solution_at_collocation_points[ind]

    # now sample random indices from the collocation points
    indices = np.random.choice(collocation_points.shape[0], num_sensor_points, replace=False)
    sensor_points = collocation_points[indices]
    sensor_values = solution_at_collocation_points[indices]

    # convert this into tensors
    sensor_points = tf.constant(sensor_points, dtype=dtype)
    sensor_values = tf.constant(sensor_values, dtype=dtype)[:, 0:2]  # Ignore the pressure values
...
if __name__ == "__main__":
    ...
    # save the sensor points as a numpy array
    np.savetxt(str(Path(i_output_path) / "sensor_points.txt"), sensor_points.numpy())
    ...
    # get the inverse parameters dict
    inverse_params_dict = datahandler_vector.get_inverse_params(get_inverse_params_dict)

    print(f"Inverse Parameters Dict = {inverse_params_dict}")

    actual_re_nr = get_inverse_params_actual_dict()["re_nr"]
    print(f"Actual Re Nr = {actual_re_nr}")
    ...
    plt.contourf(X, Y, Y_Exact_Matrix_u, levels=100, cmap="jet")
    plt.gca().set_aspect(3, adjustable="box")
    cbar = plt.colorbar(orientation='horizontal', pad=0.1, aspect=50)
    plt.scatter(sensor_points[:, 0], sensor_points[:, 1], color='black', s=3)
    plt.title("Sensor Data")
    plt.savefig(i_output_path + "/sensor_data.png")
    ...
    re_prediction_array = []  # Predicted Re Nr
    ...
    for epoch in range(num_epochs):
        ...
        re_prediction_array.append(loss['inverse_params']['re_nr'])
        ...
        if (epoch + 1) % i_update_console_output == 0 or epoch == num_epochs - 1:
            ...
            predicted_re_nr = float(loss['inverse_params']['re_nr'].numpy())
            # append test loss
            ...
            console.print(f"Sensor Loss : {sensor_loss:.3e}", end=" ")
            console.print(f"Predicted Re Nr : {predicted_re_nr:.3e}", end=" ")
            console.print(f"Actual Re Nr : {actual_re_nr:.3e}")
            ...
            plot_multiple_loss_function(
                loss_function_list=[sensor_loss_array],
                legend_labels=["Sensor Loss"],
                output_path=i_output_path,
                filename="sensor_loss_components",
                y_label="Loss",
                title="Sensor Loss Components",
                x_label="Epochs",
            )
            ...
            plot_multiple_loss_function(
                loss_function_list=[
                    re_prediction_array,
                    np.ones_like(re_prediction_array) * actual_re_nr,
                ],
                legend_labels=["Predicted Re Nr", "Actual Re Nr"],
                output_path=i_output_path,
                filename="re_nr_components",
                y_label="Re Nr",
                title="Re Nr Components",
                x_label="Epochs",
            )
    ...
    np.savetxt(str(Path(i_output_path) / "re_nr_prediction.txt"), np.array(re_prediction_array))
    # np.savetxt(str(Path(i_output_path) / "penalty_coefficients.txt"), np.array(penalty_coefficients_dict))
    ...
    os.system(f"cp inverse_backward_step.py {i_output_path}")

    # copy the model file to the output folder
    os.system(f"cp ../../../fastvpinns/model/model_nse2d_inverse_scaling.py {i_output_path}")

    # copy the ns2d file to the output folder
    os.system(f"cp ../../../fastvpinns/physics/nse2d_inverse.py {i_output_path}")
    ...
    if i_use_wandb:
        ...
        wandb.log(
            {
                "Time per Epoch": np.mean(time_array),
                "Epochs per second": int(i_epochs / np.sum(time_array)),
                "Total Train Time": np.sum(time_array),
            }
        )

        # save re_nr prediction loss
        wandb.log(
            {
                "Predicted Re Nr": predicted_re_nr,
                "Actual Re Nr": actual_re_nr,
                "Re Nr Loss": np.abs(predicted_re_nr - actual_re_nr),
            }
        )
        ...
        wandb.save(str(Path(i_output_path) / "sensor_points.txt"))
        wandb.save(str(Path(i_output_path) / "sensor_loss.txt"))
        wandb.save(str(Path(i_output_path) / "re_nr_prediction.txt"))
        ...
        wandb.save(str(Path(i_output_path) / "inverse_backward_step.py"))
        wandb.save(str(Path(i_output_path) / "model_nse2d_inverse_scaling.py"))
        wandb.save(str(Path(i_output_path) / "nse2d_inverse.py"))
