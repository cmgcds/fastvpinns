...
.. math::
   ...
   -\varepsilon\Delta u(x) + k^2 u = f(x), \quad \ x \in \Omega = (-1, 1)^2
...
conditions and boundary values, the forcing function and the exact function
...
These are usual parameters that are changed often throughout your
...
.. code:: yaml
   ...
   experimentation:
     output_path: "output/helmholtz/1"  # Path to the output directory where the results will be saved.
...
The main file ``main_helmholtz.py`` which needs to be run for the experiment, with the
...
As explained in `example file section <#example-file>`__, the
...
the ``physics`` file of ``helmholtz2d``.
