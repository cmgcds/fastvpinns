...
# Outflow Layer Example with SUPG Stabilization
...
- [Outflow Layer Example with SUPG Stabilization](#outflow-layer-example-with-l2-regularization)
  ...
  - [Input File](#input-file)
      ...
      - [model](#model)
      - [logging](#logging)
  - [Main File - main\_outflow\_supg.py](#main-file---main_cd2dpy)
...
```bash
python3 main_outflow_supg.py input.yaml
```
...
```yaml
experimentation:
  output_path: "output/outflow_supg/1"  # Path to the output directory where the results will be saved.
```
[Return to top](#contents)
...
model:
  ...
  learning_rate:  # Parameters for learning rate scheduling.
    initial_learning_rate: 0.00037037037037 # Initial learning rate.
...
## Main File - [main_outflow_supg.py](main_outflow_supg.py)
---
...
for epoch in range(num_epochs):   
        ...
        loss = model.train_step(beta=beta, bilinear_params_dict=bilinear_params_dict, l2_lambda= tf.constant(l2_lambda, dtype=i_dtype), pd_lambda =tf.constant(pd_lambda, dtype=i_dtype), \
                                    tau = tf.constant(tau, dtype=i_dtype))
...
```python
            ...
            loss_dirichlet = float(loss['loss_dirichlet'])
            ...
            console.print("[bold]tau : [/bold]" , tau, end=" ")
