...
# from model.model_hard_supg_tau_domain import *
from model.model_hard_adap_tau_parabolic import *
...
if __name__ == "__main__":
    ...
    # exdata = np.genfromtxt("./sang_ex.csv", delimiter = ',')
    # exdata = exdata[1:,:]
        ...
        #                                                * (1 - tf.exp(-k1 * (1 - test_points[:, 0:1]))) * (1 - tf.exp(-k1 * (1 - test_points[:, 1:2])))
            ...
            if i_mesh_generation_method == "internal":
                if (epoch+1) % i_update_console_output == 0 or epoch == num_epochs-1:
                    ...
                    # plot the tau
                    plot_contour_sang(x = X, y = Y, z = tau, output_path = i_output_path, filename= f"tau_{epoch+1}", title = "Tau")
    ...
    if i_mesh_generation_method == "internal":
        ...
        plot_contour_sang(x = X, y = Y, z = SUPG_Pred_Matrix, output_path = i_output_path, filename= "min_error_SUPG", title = "")
