...
# Parabolic Layer Example with SUPG Stabilization (Neural Network predicting stabilization parameter)
...
- [Mixed Boundary Layer Example with SUPG Stabilization](#mixed-boundary-layer-example-with-supg-stabilization)
  ...
  - [Input File](#input-file)
      ...
      - [model](#model)
      - [logging](#logging)
  - [Main File - main\_parabolic\_var\_tau.py](#main-file---main_parabolic_var_tau.py)
...
```bash
python3 main_paraboliv_var_tau.py input.yaml
```
...
$$
h(\mathbf{x}) := \left(1 - e^{-\kappa_1 x}\right)\left(1 - e^{-\kappa y}\right)\left(1 - e^{-\kappa(1 - x)}\right)\left(1 - e^{-\kappa(1 - y)}\right)
$$
...
```yaml
experimentation:
  output_path: "output/parabolic_var_tau/1"  # Path to the output directory where the results will be saved.
```
[Return to top](#contents)
...
## Main File - [main_parabolic_var_tau.py](main_parabolic_var_tau.py)
---

This file contains the main code to solve the mixed boundary layer problem. The code reads the input file, sets up the problem, and solves the mixed boundary layer problem using the `supg_fastvpinns` package.
...
```python
from model.model_hard_adap_tau_parabolic import DenseModel
```
...
```
- Importing the mixed boundary layer example file
```python
...
#### Reading solution from csv file
```python
...
```python
            ...
            k = 10.0/1e-8
            k1 = 30
            aa = (1 - tf.exp(-k1*test_points[:, 0:1])) * (1 - tf.exp(-k*test_points[:, 1:2])) * (1 - tf.exp(-k * (1 - test_points[:, 0:1]))) * (1 - tf.exp(-k * (1 - test_points[:, 1:2])))
