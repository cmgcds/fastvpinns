...
if __name__ == "__main__":
    ...
    for epoch in range(num_epochs):   
        ...
        if (epoch+1) % i_update_console_output == 0  or epoch == num_epochs-1:
            ...
            # console.print("[bold]k3 : [/bold]" , loss['k3'], end=" ")
            # console.print("[bold]Cw_Lambda : [/bold]" , cw_lambda, end=" ")
            ...
            plot_array(k_list, output_path = i_output_path, filename="k_value", \
                    title="k_var", x_label="Epochs", y_label="k")
            plot_array(k1_list, output_path = i_output_path, filename="k1_value", \
                    title="k1_var", x_label="Epochs", y_label="k1")
            plot_array(k2_list, output_path = i_output_path, filename="k2_value", \
                    title="k2_var", x_label="Epochs", y_label="k2")
            # plot_array(k3_list, output_path = i_output_path, filename="k3_value", \
            #         title="k3_var", x_label="Epochs", y_label="k3")
            ...
            if i_mesh_generation_method == "internal":
                if (epoch+1) % i_update_console_output == 0 or epoch == num_epochs-1:
                    ...
                    # plot the prediction
                    plot_contour_sang(x = X, y = Y, z = y_pred, output_path = i_output_path, filename= f"prediction_{epoch+1}", title = "Prediction")
                    # plot the error
                    plot_contour_sang(x = X, y = Y, z = error, output_path = i_output_path, filename= f"error_{epoch+1}", title = "Error")
        ...
        if (epoch+1) >= num_epochs - i_test_error_last_n_epochs:
            ...
            if l2_error < least_l2_error:
                ...
                with open(str(Path(i_output_path) / "min_error_summary.txt"), "w") as f:
                    ...
                    f.write(f"k at Min = {k_at_min}\n")
                    f.write(f"k1 at Min = {k1_at_min}\n")
                    f.write(f"k2 at Min = {k2_at_min}\n")
                    # f.write(f"k3 at Min = {k3_at_min}\n")
    ...
    if i_mesh_generation_method == "internal":
        ...
        plot_contour_sang(x = X, y = Y, z = Y_Pred_Matrix, output_path = i_output_path, filename= "min_error_prediction", title = "")
        # plot the final error
        plot_contour_sang(x = X, y = Y, z = np.abs(Y_Exact_Matrix - Y_Pred_Matrix), output_path = i_output_path, filename= "min_error", title = "")
