...
def pde_loss_cd2d(test_shape_val_mat, test_grad_x_mat, test_grad_y_mat, pred_nn, pred_grad_x_nn, pred_grad_y_nn, \
                    ...
                    forcing_function, real_forcing_function, bilinear_params, supg_tau, pd_lam):
    """
    ...
    # b_x*du/dx + b_y*du/dy
    single_x_y_u = bilinear_params["b_x"]* pred_grad_x_nn + bilinear_params["b_y"]* pred_grad_y_nn
    ...
    #SUPG reaction   
    supg_reaction = bilinear_params["c"]*tf.transpose(tf.linalg.matvec(single_test_x_y_v, pred_nn))
    ...
    supg_forcing = tf.transpose(tf.linalg.matvec(single_test_x_y_v, real_forcing_function))
    ...
    # supg_loss = supg_tau*(supg_residual_matrix-supg_forcing + supg_reaction)
    supg_loss = supg_residual_matrix-supg_forcing + supg_reaction  #+ supg_diffusion 
    residual_matrix = (pd_lam * pde_loss) + (supg_tau * supg_loss)
    # Perform Reduce mean along the axis 0
    residual_cells = tf.reduce_mean(tf.square(residual_matrix), axis=0)
    pde_loss = pd_lam * tf.reduce_mean(tf.square(pde_loss), axis=0)
    supg_loss = supg_tau * tf.reduce_mean(tf.square(supg_loss), axis=0)
    # residual_cells = pde_loss + supg_loss
