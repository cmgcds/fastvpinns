...
def plot_multiple_loss_function(loss_function_list, output_path, filename,  legend_labels, y_label, title, x_label="Epochs"):
    ...
    for i, (loss_function, label) in enumerate(zip(loss_function_list, legend_labels)):
        plt.plot(loss_function, label=label, alpha=0.7)# - 0.2 * i)  # Adjust alpha for transparency
...
def plot_contour_sang(x, y, z, output_path, filename, title):
    """
    This function will plot the contour plot
    """
