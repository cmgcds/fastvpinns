...
class DenseModel(tf.keras.Model):
    ...
    def call(self, inputs):
        ...
        aa = (1 - tf.exp(-10**(self.k)*tf.cast(inputs[:, 0:1], tf.float64))) * (1 - tf.exp(-10**(self.k)*tf.cast(inputs[:, 1:2], tf.float64))) * (1 - tf.exp(-10**(self.k1)*tf.cast((1 - inputs[:, 0:1]), tf.float64))) * (1 - tf.exp(-10**(self.k1)*tf.cast((1 - inputs[:, 1:2]), tf.float64)))
        x = x*aa
        return x
