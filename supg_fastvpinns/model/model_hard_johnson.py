...
class DenseModel(tf.keras.Model):
    ...
    def call(self, inputs):
        ...
        k = 10.0/0.1
        k1 = 30
        aa = tf.sin(tf.constant(math.pi)*inputs[:, 1:2])*tf.cos(tf.constant(math.pi)*inputs[:, 0:1]/2)
        bb = (1 - tf.exp(-k1*inputs[:, 1:2])) * (1-tf.exp(-k1*inputs[:, 0:1])) \
                                                   * (1 - tf.exp(-k * (1 - inputs[:, 0:1]))) * (1 - tf.exp(-k1 * (1 - inputs[:, 1:2])))
    ...
    def train_step(self, beta=10, bilinear_params_dict=None, l2_lambda = 0.0001, pd_lambda = 0.0001):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            total_loss = total_pde_loss  #+ l2_regularisation  #+ beta*boundary_loss 
        ...
        return {"loss_pde": total_pde_loss, "loss_dirichlet": boundary_loss, "loss": total_loss, "cells_residual": cells_residual, "l2_regularisation": l2_regularisation}
